...
The initrd passphrase prompt lives at `ssh -p 2222 root@<ip>`. You can wrap it in a home-manager zsh function so reboots are a single command. Two pieces (see `modules/home/shared.nix`):
...
- `foundry-unlock` — reads it back and pipes it into `ssh -tt -p 2222 root@<ip>`.
