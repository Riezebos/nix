...
- **Phase 5b** (monitoring stack — VictoriaMetrics + Loki + Alloy + Grafana): done 2026-04-22 (`modules/features/monitoring.nix`). All services bind to 127.0.0.1. Grafana now publishes through Caddy with Authentik OIDC from Phase 5f; Loki and VictoriaMetrics stay private behind Grafana's datasource proxy.
...
- **Phase 5f** (admin auth + exposure model): landed 2026-04-23 (`modules/features/authentik.nix`, `modules/features/crowdsec.nix`, Caddy/Grafana wiring, BOOTSTRAP additions). Foundry stays publicly reachable on its own auth, Grafana now has native OIDC plumbing via Authentik, Loki/VictoriaMetrics remain loopback-only, and CrowdSec + the nftables firewall bouncer watch SSH and Caddy logs. After deployment, finish the one-time Authentik application/provider bootstrap from `BOOTSTRAP.md`.
- **Phase 6** (shared PostgreSQL + logical backups): landed 2026-04-23 (`modules/features/postgresql.nix`, Authentik updated to use the shared cluster, logical dump backup wired via `services.postgresqlBackup` + restic). Authentik is now the first tenant of the shared Postgres instance; future apps should add their own `ensureDatabases` / `ensureUsers`, not a second database service.
